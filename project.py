...
def add_userdata(username, password):
	insert_query_db(
	    f'INSERT INTO userstable(username,password,usertype) VALUES ( \'{username}\',\'{password}\',\'U\' )')
	return
...
def insertUser(cid, fnampe, lnamep, dob, nationalityy, gender, email, housenum, street, city, state, zipcode, country_code, country, contact_number, nop, ec_fname, ec_lname, ec_cc, ec_contact,pspt_num,pspt_expd, c_type):
    checkEmail(email)
    sql_insert_user = 'insert into vcvt_cust (c_id, fnamep,lnamep,dob,nationalityy, gender, email, housenum, street,city, state, zipcode,country_code,country,contact_number,nop,ec_fname,ec_lname, ec_cc, ec_contact,pspt_num,pspt_expd, c_type)values ( ' + str(cid) + ',\'' + fnampe.strip() + '\' , \'' + lnamep.strip() + '\' ,\'' + str(dob) + '\',\'' + nationalityy.strip() + '\', \'' + gender + '\',\'' + email + '\',' + str(housenum) + ' , ' + str(street) + ',\'' + city.strip() + '\',\'' + state.strip() + '\',' + str(zipcode) + ',' + str(country_code) + ',\'' + country + '\',' + str(contact_number) + ',' + str(nop) + ',\'' + ec_fname.strip() + '\', \'' + ec_lname.strip() + '\',\'' + str(ec_cc) + '\',\'' + str(ec_contact) + '\',\'' + str(pspt_num) + '\',\'' +str(pspt_expd)+ '\',\'' + str(c_type) + '\');'
    try:
...
def insertPassenger(c_id,fnampe, lnamep, dob, nationalityy, gender, email, housenum, street, city, state, zipcode, country_code, country, contact_number, nop, ec_fname, ec_lname, ec_cc, ec_contact, pspt_num,pspt_expd, c_type):
    sql_insert_psngr = 'insert into vcvt_psngr(c_id,fnamep,lnamep,dob,nationalityy, gender, email, housenum, street,city, state, zipcode,country_code,country,contact_number,nop,ec_fname,ec_lname, ec_cc, ec_contact, pspt_num,pspt_expd, c_type)values ( \''+str(c_id)+ '\', \''+fnampe.strip() + '\' , \'' + lnamep.strip() + '\' ,\'' + str(dob) + '\',\'' + nationalityy.strip() + '\', \'' + gender + '\',\'' + email + '\',' + str(housenum) + ' , ' + str(street) + ',\'' + city.strip() + '\',\'' + state.strip() + '\',' + str(zipcode) + ',' + str(country_code) + ',\'' + country + '\',' + str(contact_number) + ',' + str(nop) + ',\'' + ec_fname.strip() + '\', \'' + ec_lname.strip() + '\',\'' + str(ec_cc) + '\',\'' + str(ec_contact) + '\',\'' + str(pspt_num) + '\',\'' +str(pspt_expd)+ '\',\'' + str(c_type) + '\');'
    try:
...
def main():
                    ...
                    if choice == 'New Customer':
                        if existing == 0:
                            ...
                            if st.button('Submit'):
                                ...
                                insertUser(query_db(sql_get_cust_id)['customerid'].loc[0], fname, lname, dob, nationalityy, gender, email, housenum, street,city, state, zipcode,country_code,country,contact_number,nop,ec_fname,ec_lname, ec_cc, ec_contact,pspt_num,pspt_expd, c_type  )
                                # restaurants = query_db(sql_find_rest)
                                # planid = st.selectbox('Select a plan:' ,  restaurants['name'].tolist())
                        elif existing ==1:
                            sql_get_cust_id = f'select customerID from userstable where username = \'{username}\''
                            st.write('You are already registered and your customer id is ' + str(query_db(sql_get_cust_id)['customerid'].loc[0]) )
                        ...
                        if c_type:
                            if c_type == 'Booking Agent':
                                ...
                                c_type = 'C'
                        if st.button('Submit'):
                            insertPassenger(c_id,fname, lname, dob, nationalityy, gender, email, housenum, street,city, state, zipcode,country_code,country,contact_number,nop,ec_fname,ec_lname, ec_cc, ec_contact,pspt_num,pspt_expd, c_type )
                        ...
                        if flightid:
                            ...
                            st.dataframe(flightdetail.loc[flightdetail['f_id']==flightid].rename(columns={"f_id":"Flight ID", "d_time":"Departure time","d_tz":"Departure Time zone","arr_time":"Arrival time", "a_tz":"Arrival time zone","am_id":"Aircraft Model","arpt_code_arv":"Arrival airport code","arpt_code_dep":"Departure airport code" }))
    ...
    # adding at the submit button
                        if st.button('Submit'): 
                            ...
                            for i in speREq:
                                ...
                                insert_query_db(sql_psngr_flight)                             
                            st.write('Flight Detail Added. To add the connecting flight, please add flight detail again')
    ...
    # SQL Injection 
                        sql_get_customer_id = 'select customerID from userstable where username = {usernm}'.format(usernm=sql.Placeholder(username))
